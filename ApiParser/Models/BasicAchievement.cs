...
namespace VideoGamesSpa.ApiParser.Models
{
	...
	public class BasicAchievement
	{
		...
		/*public bool UpdateHiddenAchievement(string achievementId, string gameId)
		{
			if (string.IsNullOrWhiteSpace(this.Title) && this.Earned.HasValue)
			{
				var hiddenAchievement = HiddenAchievement.ParseHiddenAchievementsXml()
		...
		/// <param name="gameId">Id of the game.</param>
		/// <returns>True if the achievement was updated.</returns>
		/*public bool UpdateOfflineAchievement(string achievementId, string gameId)
		{
			if (this.Earned.HasValue && this.Earned.Value.Year == 1)
			{
				var offlineAchievement = OfflineAchievement.OfflineAchievements()
					.FirstOrDefault(a => a.GameId == gameId && a.Id == achievementId);
